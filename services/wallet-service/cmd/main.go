...
import (
	...
	"github.com/example/fintech-backend/services/wallet-service/internal/app"
...
func main() {
	...
	walletService := app.NewService(repo, redisClient, publisher)
	...
	app.NewHTTPHandler(walletService).RegisterRoutes(r.Group("/api/v1/wallets"))
	...
	walletpb.RegisterWalletServiceServer(grpcServer, app.NewGRPCServer(walletService))
	...
	go func() {
		log.Info("wallet grpc server started", zap.String("port", cfg.GRPCPort))
		if serveErr := grpcServer.Serve(lis); serveErr != nil {
			log.Fatal("grpc serve failed", zap.Error(serveErr))
		}
	}()
	go func() {
		log.Info("wallet http server started", zap.String("port", cfg.HTTPPort))
